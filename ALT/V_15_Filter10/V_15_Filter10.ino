...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    float FehlerV = 4.000 *min_steigen;     //Gewichtung fuer Vario Filter berechnen. 0.1 > FehlerV < 1.0
...
float kal[11], Vario, VarioR, Hoehe, Temp, AvrgV = 0.000;

unsigned long  dZeit, ZeitE, ZeitS, ZeitPipA, ZeitPipE;
...
void SteigenBerechnen()
{
      ...
      // Steigwerte berechnen.
        Vario=((Hoehe - kal[1]) / (float(dZeit)/1000000));
        VarioR=Vario;
        kal[1]=Hoehe;
      ...
      // < Mittelwert bilden.
        ...
        Vario = FehlerV*Vario + (1-FehlerV)*kal[0];
        kal[0] = Vario;
      ...
      if ( Vario >= min_steigen)
        {
            if ( Vario <= 7  )
            {
                if ( (millis() - ZeitPipA) >= (unsigned long)(2*duration) ) 
                {
                    ZeitPipA = millis();
